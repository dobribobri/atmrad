...
def brightness_temperatures(frequencies: Union[np.ndarray, List[float]], atm: 'Atmosphere',
                            srf: 'Surface', cosmic: bool = True, n_workers: int = None) -> np.ndarray:
    """
    Яркостная температура уходящего излучения системы 'атмосфера - подстилающая поверхность'
    ...
    """
    return parallel(frequencies, func=brightness_temperature, args=(atm, srf, cosmic, ), n_workers=n_workers)
