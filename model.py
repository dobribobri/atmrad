...
class StandardProfiles:
    ...
    def altitudes(self) -> Union[np.ndarray, tf.Tensor]:
    ...
    @staticmethod
    def gpu(profile: np.ndarray) -> Union[np.ndarray, tf.Tensor]:
        if gpu:
    ...
    def zeros(self) -> Union[np.ndarray, tf.Tensor]:
        return self.gpu(self.reshape(np.zeros(len(self._altitudes), dtype=cpu_float), (self.nx, self.ny)))

    def T(self, celsius=True) -> Union[np.ndarray, tf.Tensor]:
        """
    ...
    def P(self, HP: float = 7.7) -> Union[np.ndarray, tf.Tensor]:
        """
    ...
    def rho(self, Hrho: float = 2.1) -> Union[np.ndarray, tf.Tensor]:
        """
...
class Domain:
    ...
    def liquid_water_distribution(self, height_map: np.ndarray, const_w=False,
                                  mu0: float = 3.27, psi0: float = 0.67) -> Union[np.ndarray, tf.Tensor]:
...
class Model:
    ...
    class compat:
        ...
        @staticmethod
        def rank(a: Union[np.ndarray, tf.Tensor]) -> int:
            if isinstance(a, cpu_types):
                return a.ndim
            return tf.rank(a)

        @staticmethod
        def sum(a: Union[np.ndarray, tf.Tensor], axis: int = None) -> Union[np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.sum(a, axis=axis)
            return tf.reduce_sum(a, axis=axis)

        @staticmethod
        def transpose(a: Union[np.ndarray, tf.Tensor], axes=None) -> Union[np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.transpose(a, axes)
            return tf.transpose(a, perm=axes)

        @staticmethod
        def len(a: Union[np.ndarray, tf.Tensor]) -> int:
            if isinstance(a, cpu_types):
                return a.shape[-1]
            return tf.shape(a)[-1]

        @staticmethod
        def last(a: Union[np.ndarray, tf.Tensor]) -> int:
            return Model.compat.len(a) - 1

        @staticmethod
        def integrate(a: Union[np.ndarray, tf.Tensor],
                      dh: float, method='trapz') -> Union[float, np.ndarray, tf.Tensor]:
            return Model.compat.integrate_bounds(a, 0, Model.compat.last(a), dh, method)

        @staticmethod
        def integrate_bounds(a: Union[np.ndarray, tf.Tensor], lower: int, upper: int,
                             dh: float, method='trapz') -> Union[float, np.ndarray, tf.Tensor]:
        ...
        def integrate_callable(f: Callable, lower: int, upper: int,
                               dh: float) -> Union[float, np.ndarray, tf.Tensor]:
        ...
        @staticmethod
        def exp(a: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.exp(a)
            return tf.exp(a)

        @staticmethod
        def sin(a: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.sin(a)
            return tf.sin(a)

        @staticmethod
        def cos(a: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.cos(a)
            return tf.cos(a)

        @staticmethod
        def sqrt(a: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.sqrt(a)
            return tf.sqrt(a)

        @staticmethod
        def abs(a: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
            if isinstance(a, cpu_types):
                return np.absolute(a)
            return tf.abs(a)

        @staticmethod
        def to_numpy_array(a: Union[float, np.ndarray, tf.Tensor], dtype: Any = cpu_float) -> np.ndarray:
            if not (isinstance(a, cpu_types)):
                a = a.numpy()
            return np.asarray(a, dtype=dtype)

        @staticmethod
        def to_tensor(a: Union[float, np.ndarray, tf.Tensor], dtype: Any = gpu_float) -> tf.Tensor:
        ...
        @staticmethod
        def at(a: Union[np.ndarray, tf.Tensor], index: int) -> Union[np.ndarray, tf.Tensor]:
            rank = Model.compat.rank(a)
            if rank not in [1, 3]:
                raise RuntimeError('неверная размерность')
            if rank == 3:
    ...
    def gamma_oxygen(frequency: float,
                     T: Union[float, np.ndarray, tf.Tensor],
                     P: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
        """
        ...
        if f <= 57:
            ...
            gamma = (f - 60) * (f - 63) / 18 * Model.gamma_oxygen(57., T, P) - \
                    1.66 * rp * rp * np.power(rt, 8.5) * (f - 57) * (f - 63) + \
                    (f - 57) * (f - 60) / 18 * Model.gamma_oxygen(63., T, P)
        return gamma

    @staticmethod
    def gamma_water_vapor(frequency: float,
                          T: Union[float, np.ndarray, tf.Tensor],
                          P: Union[float, np.ndarray, tf.Tensor],
                          rho: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
        """
    ...
    def tau_oxygen_near_ground(frequency: float,
                               T_near_ground: Union[float, np.ndarray, tf.Tensor],
                               P_near_ground: Union[float, np.ndarray, tf.Tensor],
                               theta: float = 0.0) -> Union[float, np.ndarray, tf.Tensor]:
        """
        Учитывает угол наблюдения.

        :return: полное поглощение в кислороде (модельн.). В неперах.
        """
        gamma = Model.gamma_oxygen(frequency, T_near_ground, P_near_ground)
        return gamma * Model.H1(frequency) / np.cos(theta) * dB2np

    @staticmethod
    def tau_water_vapor_near_ground(frequency: float,
                                    T_near_ground: Union[float, np.ndarray, tf.Tensor],
                                    P_near_ground: Union[float, np.ndarray, tf.Tensor],
                                    rho_near_ground: Union[float, np.ndarray, tf.Tensor],
                                    theta: float = 0.0, rainQ=False) -> Union[float, np.ndarray, tf.Tensor]:
        """
        Учитывает угол наблюдения.

        :return: полное поглощение в водяном паре (модельн.). В неперах
        """
        gamma = Model.gamma_water_vapor(frequency, T_near_ground, P_near_ground, rho_near_ground)
        return gamma * Model.H2(frequency, rainQ=rainQ) / np.cos(theta) * dB2np

    @staticmethod
    def tau_oxygen_integral(frequency: float,
                            T: Union[np.ndarray, tf.Tensor],
                            P: Union[np.ndarray, tf.Tensor],
                            dh: float) -> Union[float, np.ndarray, tf.Tensor]:
        """
        :return: полное поглощение в кислороде (путем интегрирования). В неперах.
        """
        gamma = Model.gamma_oxygen(frequency, T, P)
        return dB2np * Model.compat.integrate(gamma, dh)

    @staticmethod
    def tau_water_vapor_integral(frequency: float,
                                 T: Union[np.ndarray, tf.Tensor],
                                 P: Union[np.ndarray, tf.Tensor],
                                 rho: Union[np.ndarray, tf.Tensor],
                                 dh: float) -> Union[float, np.ndarray, tf.Tensor]:
        """
        :return: полное поглощение в водяном паре (путем интегрирования). В неперах.
        """
        gamma = Model.gamma_water_vapor(frequency, T, P, rho)
        return dB2np * Model.compat.integrate(gamma, dh)

    @staticmethod
    def krho(frequency: float,
             T_standard: Union[np.ndarray, tf.Tensor],
             P_standard: Union[np.ndarray, tf.Tensor],
             rho_standard: Union[np.ndarray, tf.Tensor], dh: float) -> float:
        return Model.tau_water_vapor_integral(frequency, T_standard, P_standard, rho_standard, dh) / \
               (Model.compat.integrate(rho_standard, dh) / 10.)

    @staticmethod
    def epsilon(T: Union[float, np.ndarray, tf.Tensor],
                Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Tuple[float, float, float]:
        epsO_nosalt = 5.5
    ...
    def kw(frequency: float, t_cloud: float) -> float:
        ...
        epsO, epsS, lambdaS = Model.epsilon(t_cloud)
        y = lambdaS / lamda
    ...
    def gamma_liquid_water(frequency: float, t_cloud: float,
                           w: Union[float, np.ndarray, tf.Tensor]) -> Union[float, np.ndarray, tf.Tensor]:
        if isinstance(w, (float, np.ndarray)):
            return np2dB * Model.kw(frequency, t_cloud) * w
        return Model.compat.to_tensor(np2dB * Model.kw(frequency, t_cloud)) * w

    @staticmethod
    def tau_liquid_water_integral():
        pass

    @staticmethod
    def gamma_summary(frequency: float,
                      T: Union[float, np.ndarray, tf.Tensor], P: Union[float, np.ndarray, tf.Tensor],
                      rho: Union[float, np.ndarray, tf.Tensor], w: Union[float, np.ndarray, tf.Tensor] = None,
                      t_cloud: float = None) -> Union[float, np.ndarray, tf.Tensor]:
        if w is None or t_cloud is None:
            return dB2np * (Model.gamma_oxygen(frequency, T, P) + Model.gamma_water_vapor(frequency, T, P, rho))
        return dB2np * (Model.gamma_oxygen(frequency, T, P) + Model.gamma_water_vapor(frequency, T, P, rho) +
                        Model.gamma_liquid_water(frequency, t_cloud, w))
    ...
    @staticmethod
    def epsilon_complex(frequency: float, T: Union[float, np.ndarray, tf.Tensor],
                        Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[complex, np.ndarray, tf.Tensor]:
        lamda = C / (frequency * 10 ** 9) * 100  # перевод в [cm]
    ...
    @staticmethod
    def M_horizontal(frequency: float, psi: float, T: Union[float, np.ndarray, tf.Tensor],
                     Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[float, np.ndarray, tf.Tensor]:
        epsilon = Model.epsilon_complex(frequency, T, Sw)
        cos = Model.compat.sqrt(epsilon - np.cos(psi) * np.cos(psi))
        return (np.sin(psi) - cos) / (np.sin(psi) + cos)

    @staticmethod
    def M_vertical(frequency: float, psi: float, T: Union[float, np.ndarray, tf.Tensor],
                   Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[float, np.ndarray, tf.Tensor]:
        epsilon = Model.epsilon_complex(frequency, T, Sw)
        cos = Model.compat.sqrt(epsilon - np.cos(psi) * np.cos(psi))
    ...
    @staticmethod
    def R_horizontal(frequency: float, theta: float, T: Union[float, np.ndarray, tf.Tensor],
                     Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[float, np.ndarray, tf.Tensor]:
    ...
    @staticmethod
    def R_vertical(frequency: float, theta: float, T: Union[float, np.ndarray, tf.Tensor],
                   Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[float, np.ndarray, tf.Tensor]:
    ...
    @staticmethod
    def R(frequency: float, T: Union[float, np.ndarray, tf.Tensor],
          Sw: Union[float, np.ndarray, tf.Tensor] = 0.) -> Union[float, np.ndarray, tf.Tensor]:
        epsilon = Model.epsilon_complex(frequency, T, Sw)
    ...
    class Downward:
        ...
        def brightness_temperature(frequency: float,
                                   T: Union[np.ndarray, tf.Tensor], P: Union[np.ndarray, tf.Tensor],
                                   rho: Union[np.ndarray, tf.Tensor], w: Union[np.ndarray, tf.Tensor],
                                   t_cloud: float, dh: float, TK: float = 0.) -> Union[float, np.ndarray, tf.Tensor]:
            g = Model.gamma_summary(frequency, T, P, rho, w, t_cloud)
            f = lambda h: (Model.compat.at(T, h) + 273.15) * Model.compat.at(g, h) * \
                Model.compat.exp(-Model.compat.integrate_bounds(g, 0, h, dh))
            inf = Model.compat.last(g)
        ...
        class Multifreq:
            ...
            def brightness_temperature(
                    ...
                    T: Union[np.ndarray, tf.Tensor], P: Union[np.ndarray, tf.Tensor],
                    rho: Union[np.ndarray, tf.Tensor], w: Union[np.ndarray, tf.Tensor],
                    t_cloud: float, dh: float, TK: float = 0., n_workers: int = None) -> np.ndarray:
    ...
    class Upward:
        ...
        def brightness_temperature(frequency: float,
                                   T: Union[np.ndarray, tf.Tensor], P: Union[np.ndarray, tf.Tensor],
                                   rho: Union[np.ndarray, tf.Tensor], w: Union[np.ndarray, tf.Tensor],
                                   t_cloud: float, dh: float) -> Union[float, np.ndarray, tf.Tensor]:
            g = Model.gamma_summary(frequency, T, P, rho, w, t_cloud)
            inf = Model.compat.last(g)
            f = lambda h: (Model.compat.at(T, h) + 273.15) * Model.compat.at(g, h) * \
    ...
    class Satellite:
        @staticmethod
        def brightness_temperature(frequency: float,
                                   Tsurface: Union[float, np.ndarray, tf.Tensor],
                                   Salinity: Union[float, np.ndarray, tf.Tensor],
                                   T: Union[np.ndarray, tf.Tensor], P: Union[np.ndarray, tf.Tensor],
                                   rho: Union[np.ndarray, tf.Tensor], w: Union[np.ndarray, tf.Tensor],
                                   t_cloud: float, dh: float,
                                   TK: float = 0.) -> Union[float, np.ndarray, tf.Tensor]:
            g = Model.gamma_summary(frequency, T, P, rho, w, t_cloud)
            inf = Model.compat.last(g)
            ...
            opacity = Model.compat.exp(-Model.compat.integrate(g, dh))
            Tb_down: Union[float, np.ndarray, tf.Tensor] = \
                Model.compat.integrate_callable(f_down, 0, inf, dh) + TK * opacity
            Tb_up: Union[float, np.ndarray, tf.Tensor] = \
                Model.compat.integrate_callable(f_up, 0, inf, dh)

            R = Model.R(frequency, Tsurface, Salinity)
            kappa = 1. - R
            if isinstance(g, (float, np.ndarray)):
                return (Tsurface + 273.15) * kappa * opacity + Tb_up + R * Tb_down * opacity
            return Model.compat.to_tensor((Tsurface + 273.15) * kappa) * opacity + Tb_up + \
                Model.compat.to_tensor(R) * Tb_down * opacity
        ...
        class Multifreq:
            ...
            def brightness_temperature(frequencies: Union[np.ndarray, List[float], Iterable[float]],
                                       ...
                                       rho: Union[np.ndarray, tf.Tensor], w: Union[np.ndarray, tf.Tensor],
                                       t_cloud: float, dh: float,
                                       TK: float = 0., n_workers: int = None) -> Union[float, np.ndarray, tf.Tensor]:
                if Model.compat.are_numpy_arrays(T, P, rho, w):
                    return Model.multi.parallel(frequencies,
...
if __name__ == '__main__':
    ...
    start_time = time.time()
    # brt = Model.Upward.Multifreq.brightness_temperature([18.0, 22.2, 27.2], T, P, rho, w, -2, dh)
